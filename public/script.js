...
const translations = {
  fr: {
    ...
    siteName: 'Rhum Danou', emptyCart: '🛒 Panier vide', emptyPosts: '🥃 Aucune publication',
  ...
  en: {
    ...
    siteName: 'Rhum Danou', emptyCart: '🛒 Empty cart', emptyPosts: '🥃 No posts',
  ...
  ht: {
    ...
    siteName: 'Rhum Danou', emptyCart: '🛒 Panyen vid', emptyPosts: '🥃 Pa gen piblikasyon',
    navHome: 'Akèy', navCart: 'Panyen', navMessages: 'Mesaj', navCreator: 'Kreyatè', navAdmin: 'Admin',
    cartTitle: '🛒 Panyen mwen', messagingTitle: '💬 Mesajri', adminTitle: '⚙️ Administrasyon',
    adminPublish: '📸 Pibliye medya', adminText: '📝 Mesaj tèks', adminUsers: '👥 Itilizatè',
    publishBtn: '✨ Pibliye', publishMessageBtn: '📢 Pibliye mesaj',
    creatorProfile: '👑 Pwofil Kreyatè', logout: '🚪 Dekonekte', loading: 'Chajman...',
    placeholder: 'Ekri mesaj w...'
  },
  es: {
    ...
    siteName: 'Rhum Danou', emptyCart: '🛒 Carrito vacío', emptyPosts: '🥃 Sin publicaciones',
    navHome: 'Inicio', navCart: 'Carrito', navMessages: 'Mensajes', navCreator: 'Creador', navAdmin: 'Admin',
    cartTitle: '🛒 Mi Carrito', messagingTitle: '💬 Mensajería', adminTitle: '⚙️ Administración',
    adminPublish: '📸 Publicar medio', adminText: '📝 Mensaje texto', adminUsers: '👥 Usuarios',
    publishBtn: '✨ Publicar', publishMessageBtn: '📢 Publicar mensaje',
...
// Conversations pour la messagerie
let conversations = []; // [{ userId, userName, userAvatar, messages, unreadCount, lastMessage, lastMessageTime }]
...
function applyLanguage() {
  ...
  const elements = {
    ...
    adminPublishTab: 'adminPublishTab', adminTextTab: 'adminTextTab', adminUsersTab: 'adminUsersTab',
    publishBtn: 'publishBtn', publishMessageBtn: 'publishMessageBtn',
    creatorProfileBtn: 'creatorProfileBtn', logoutBtn: 'logoutBtn', loadingText: 'loadingText',
    navHomeText: 'navHomeText', navCartText: 'navCartText', navMessagesText: 'navMessagesText',
    navCreatorText: 'navCreatorText', navAdminText: 'navAdminText'
  };
  ...
  if (document.getElementById('panel-messaging')?.classList.contains('active')) renderConversationsList();
}
...
// ==================== CHAT INTELLIGENT ====================
function initChat() {
  const welcomeMsg = translations[userLang]?.welcomeMessage || '👋 Bonjour ! Je suis votre assistant Rhum Danou.';
  chatHistory = [{ role: 'assistant', content: welcomeMsg }];
  const chatWindow = document.getElementById('chatWindow');
  if (chatWindow) {
    chatWindow.innerHTML = `<div class="msg-bubble msg-bot">${welcomeMsg}</div>`;
  }
}
...
async function sendSmartMessage() {
  ...
  if (!text) return;
  input.value = '';
  // Fonctionnalité chat à implémenter si besoin
  showToast('Message envoyé à l\'assistant');
}
  ...
  container.innerHTML = conversations.map(conv => `
    <div class="conversation-item" onclick="openConversation('${conv.userId}')">
      <div class="conversation-avatar">
        ${conv.userAvatar ? `<img src="${conv.userAvatar}">` : conv.userName?.[0]?.toUpperCase() || '?'}
      </div>
      <div class="conversation-info">
        <div class="conversation-name">${escapeHtml(conv.userName)}</div>
        <div class="conversation-last-msg">${escapeHtml((conv.lastMessage || '').substring(0, 50))}</div>
      </div>
      ${conv.unreadCount > 0 ? `<div class="conversation-unread">${conv.unreadCount}</div>` : ''}
      <div class="conversation-time">${conv.lastMessageTime ? timeAgo(conv.lastMessageTime) : ''}</div>
    </div>
  `).join('');
}
...
function openConversation(userId) {
  ...
  document.getElementById('currentConversationName').textContent = conv?.userName || 'Discussion';
}
...
function renderMessagesForUser(userId) {
  ...
  container.innerHTML = conv.messages.map(m => `
    <div class="partner-message ${m.senderId === 'creator' ? 'partner-message-creator' : 'partner-message-user'}">
      ...
      ${m.media ? (m.mediaType === 'audio' ? `<audio controls src="${m.media}" class="partner-message-media"></audio>` : `<img src="${m.media}" class="partner-message-media">`) : ''}
...
async function sendPartnerMessageToCurrent() {
  if (!currentConversationUserId) {
    showToast('Sélectionnez une conversation');
    return;
  }
...
// Pour les utilisateurs normaux qui veulent envoyer un message au créateur
function sendMessageToCreator(text, media, mediaType) {
  let conv = conversations.find(c => c.userId === 'creator');
  if (!conv) {
    conv = {
      userId: 'creator',
      userName: 'Danou (Créateur)',
      userAvatar: null,
      messages: [],
      unreadCount: 0,
      lastMessage: '',
  ...
  saveConversations();
  showToast('Message envoyé au créateur');
}
  ...
  el.innerHTML = users.map(u => `
    <div class="user-row">
      ...
      <div class="user-actions">
        ...
        ${currentUser?.role === 'creator' || currentUser?.role === 'admin' ? `<button class="btn-xs btn-promote" onclick="startConversationWithUser('${u.id}', '${escapeHtml(u.name)}')">💬 Message</button>` : ''}
      </div>
    </div>`).join('');
}
...
function showPanel(name) {
  ...
  const btnId = name === 'messaging' ? 'bnav-messaging' : `bnav-${name}`;
  ...
  if (name === 'admin' && currentUser?.role !== 'creator' && currentUser?.role !== 'admin') showPanel('feed');
