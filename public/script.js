// ==================== FICHIER script.js COMPLET CORRIGÉ ====================
...
async function init() {
  ...
  // Charger le feed public immédiatement sans connexion
  await loadFeedPublic();
  loadStories();
  updateCartBadge();

  // Si session active (créateur/admin), charger les fonctions avancées
  try {
...
// Chargement du feed sans connexion (lecture seule)
async function loadFeedPublic() {
  try {
    ...
    document.getElementById('feedContent').innerHTML = '<div class="empty-state">Erreur de chargement</div>';
  }
}
...
async function enterAppConnected() {
  ...
  // Switcher le dropdown
  document.getElementById('profileDropdownConnected').style.display = 'block';
  document.getElementById('profileDropdownGuest').style.display = 'none';
...
async function doLogout() {
  ...
  currentUser = null;
  // Cacher les boutons réservés créateur/admin
  document.getElementById('bnav-creator').style.display = 'none';
  document.getElementById('bnav-admin').style.display = 'none';
  // Revenir au dropdown visiteur
  document.getElementById('profileDropdownConnected').style.display = 'none';
  document.getElementById('profileDropdownGuest').style.display = 'block';
...
function renderPost(post) {
  ...
  const commentsHTML = (post.comments || []).slice(-3).map(c => `<div class="comment-item"><span><strong>${escapeHtml(c.userName)}</strong> ${escapeHtml(c.text)}</span>${userIsAdmin ? `<button class="del-comment-btn" onclick="deleteComment('${post.id}','${c.id}')">✕</button>` : ''}</div>`).join('');
  const adminControls = userIsAdmin ? `<div class="admin-post-controls"><button class="btn-admin-sm btn-edit" onclick="openEditModal('${post.id}')">✏️ Modifier</button><button class="btn-admin-sm btn-delete" onclick="deletePost('${post.id}')">🗑 Supprimer</button></div>` : '';
...
function orderViaWhatsApp() {
  // Ouvrir le modal de livraison avec les questions automatiques
  openDeliveryModal();
}
...
function confirmOrder() {
  ...
  const waNumber = (creatorLinks.phone || '50933324695').replace(/[^0-9]/g, '');
...
function renderConversationsList() {
  ...
  if (!isAdmin()) {
    visibleConversations = conversations.filter(c => c.userId === currentUser?.id);
  }
...
function renderMessagesForUser(userId) {
  ...
  container.innerHTML = conv.messages.map(m => `
    <div class="partner-message ${m.senderId === 'creator' ? 'partner-message-creator' : 'partner-message-user'}">
      <div><strong>${escapeHtml(m.senderName)}</strong> <span style="font-size:0.6rem">${new Date(m.createdAt).toLocaleTimeString()}</span></div>
...
async function renderCreatorProfile() {
  ...
  // Compteur visiteurs réels depuis le serveur
  ...
  el.innerHTML = `
    ...
    <div class="profile-stats">
      <div class="stat-item"><div class="stat-number">${totalVisitors}</div><div class="stat-label">Visiteurs</div></div>
    ...
    <div class="creator-status">
      ...
      ${isCreator() ? '<button class="btn-primary" onclick="editCreatorStatus()" style="margin-top:0.5rem;">✏️ Modifier</button>' : ''}
    </div>
  `;
}
...
function applyLanguage() {
  ...
  const elements = {
    ...
    creatorProfileBtn: 'creatorProfileBtn', logoutBtn: 'logoutBtn'
  };
...
function showPanel(name) {
  ...
  if (name === 'admin' && !isAdmin()) showPanel('feed');
  document.getElementById('profileDropdown')?.classList.remove('open');
  document.getElementById('themeLangMenu')?.classList.remove('open');
}
...
async function sendChatMessage() {
  ...
  try {
    ...
    typingDiv.textContent = 'Erreur de connexion.';
  }
}
